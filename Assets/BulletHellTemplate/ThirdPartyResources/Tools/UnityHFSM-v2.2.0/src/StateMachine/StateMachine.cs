...
namespace HFSM
{
		...
		IActionable<TEvent>
	{
		...
		{
			if (activeState == null)
				throw HFSM.Exceptions.Common.NotInitialized(this, context);
		}
		...
		private void ChangeState(TStateId name, ITransitionListener listener = null)
		{
			...
			{
				throw HFSM.Exceptions.Common.StateNotFound(this, name.ToString(), context: "Switching states");
			}
		...
		public override void OnEnter()
		{
			...
			{
				throw HFSM.Exceptions.Common.MissingStartState(this, context: "Running OnEnter of the state machine.");
			}
		...
		public StateBase<TStateId> GetState(TStateId name)
		{
			...
			if (!stateBundlesByName.TryGetValue(name, out bundle) || bundle.state == null)
			{
				throw HFSM.Exceptions.Common.StateNotFound(this, name.ToString(), context: "Getting a state");
			}
		...
		public StateMachine<string, string, string> this[TStateId name]
		{
			get
			{
				...
				if (subFsm == null)
				{
					throw HFSM.Exceptions.Common.QuickIndexerMisusedForGettingState(this, name.ToString());
				}
		...
		public TStateId GetStartStateName()
		{
			if (!startState.hasState)
			{
				throw HFSM.Exceptions.Common.MissingStartState(
					this,
